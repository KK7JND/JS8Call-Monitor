...
class StatusPacket(GenericWSJTXPacket):
    ...
    @classmethod
    # make a status packet (a byte array) we can send to a 'client'. This should be it's own class.
    def Builder(cls,wsjtx_id='pywsjtx', dial_frequency=0, mode="", dx_call="", report="", tx_mode="", tx_enabled=False, transmitting=False, decoding=False, rx_df=0, tx_df=0, de_call="", de_grid="", dx_grid="", tx_watchdog=False, sub_mode="", fast_mode=False, special_op_mode=0):
        # build the packet to send
        pkt = PacketWriter()
...
class DecodePacket(GenericWSJTXPacket):
    ...
    @classmethod
    # make a decode packet (a byte array) we can send to a 'client'. This should be it's own class.
    def Builder(cls,wsjtx_id='pywsjtx', new=False, time=0, snr=0, delta_time=0.0, delta_frequency=0, mode="", message="", low_confidence=False, off_air=False):
        # build the packet to send
        pkt = PacketWriter()
        pkt.write_QInt32(DecodePacket.TYPE_VALUE)
        pkt.write_QString(wsjtx_id)
        pkt.write_QInt8(new)
        pkt.write_QInt32(time)
        pkt.write_QInt32(snr)
        pkt.write_QFloat(delta_time)
        pkt.write_QInt32(delta_frequency)
        pkt.write_QString(mode)
